...
module equalizer_systolic #(
    ...
    output reg  signed [18:0] data_out        // 输出信号，19 位有符号整数，保留 Q6 精度
);

    integer i;
    
    // 乘法器的输出
    wire signed [15:0] multi_full [0:7]; // 8 个乘法器的完整输出，每个输出 16 位
    wire signed [11:0] multi_out [0:7];  // 截去 4 个低位小数后的乘法输出，每个输出 12 位
    // 加法器输出
    wire signed [18:0] add_out [0:6]; // 7 个加法器的输出，每个输出 19 位
        // 10 级移位寄存器，打断串行加法器的关键路径
    reg signed [18:0] shift_data [0:9];
    ...
    // 乘法器逻辑：输入广播到所有乘法器
    assign multi_full[0] = data_in_reg * multi_coeffs_0; // x[n] * h[0]
    assign multi_full[1] = data_in_reg * multi_coeffs_1; // x[n] * h[1]
    assign multi_full[2] = data_in_reg * multi_coeffs_2; // x[n] * h[2]
    assign multi_full[3] = data_in_reg * multi_coeffs_3; // x[n] * h[3]
    assign multi_full[4] = data_in_reg * multi_coeffs_4; // x[n] * h[4]
    assign multi_full[5] = data_in_reg * multi_coeffs_5; // x[n] * h[5]
    assign multi_full[6] = data_in_reg * multi_coeffs_6; // x[n] * h[6]
    assign multi_full[7] = data_in_reg * multi_coeffs_7; // x[n] * h[7]
    assign multi_out[0] = multi_full[0][15:4];
    assign multi_out[1] = multi_full[1][15:4];
    assign multi_out[2] = multi_full[2][15:4];
    assign multi_out[3] = multi_full[3][15:4];
    assign multi_out[4] = multi_full[4][15:4];
    assign multi_out[5] = multi_full[5][15:4];
    assign multi_out[6] = multi_full[6][15:4];
    assign multi_out[7] = multi_full[7][15:4];
