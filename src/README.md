...
## III. 电路优化技术与 RTL 设计
在 RTL 硬件实现层面，滤波器的核心是离散卷积带来的繁重乘加操作（MAC）。本项目利用基本型与不同经典的电路优化技术探索了多种架构：
...
**Lite 位宽裁剪版本**：`rtl/folding_lite/equalizer_folding.v` 在保持折叠控制、移位寄存器和时分复用乘加结构不变的前提下，只对乘法和累加数据通路做位宽缩减。原始 INT8 与 INT8 相乘后得到 16 bit Q10 乘积，Lite 版本截去乘积低 4 位小数，将后续计算统一降为 Q6：乘法输出寄存器由 16 bit 缩减为 12 bit，累加器与 `data_out` 由 23 bit 缩减为 19 bit。该版本牺牲部分小数精度以减少乘法结果寄存器、加法器和输出端口的面积开销，testbench 仍复用 `equalizer_folding_tb.v`，仿真时使用 `--frac-bits 6` 解释输出结果。
...
**Lite 位宽裁剪版本**：`rtl/systolic_array_lite/equalizer_systolic.v` 保持原脉动阵列的数据流、寄存器传递关系和 valid 时序不变，仅在每个处理单元的乘法输出处加入统一截位。每个 16 bit Q10 乘积取 `[15:4]` 作为 12 bit Q6 中间结果，阵列内部的移位累加寄存器、加法器输出和最终 `data_out` 相应改为 19 bit Q6。由于脉动阵列包含 8 个并行乘法路径和 7 个加法路径，该位宽缩减能直接降低并行数据通路的寄存器与加法器资源；testbench 继续复用 `equalizer_systolic_tb.v`，仿真时同样使用 `--frac-bits 6`。

### 3.5 展开架构 (Unfolding Architecture)

**Lite 位宽裁剪版本**：相乘得到 16 bit 乘积，其中包含 10 bit 小数。Lite 版本保留乘积的高 13 bit，截去低 3 bit 小数位，牺牲一部分量化精度以减少后续加法树的操作数位宽。
- RTL 路径为 `rtl/unfolding_lite/equalizer_unfolding_n2.v`，模块接口保持与原 `unfolding` 版本一致，仅将 `data_out_even` 和 `data_out_odd` 从 19 bit 降为 16 bit。
- 偶路和奇路仍各包含 8 项乘法，先生成 16 bit 原始乘积，再取 `[15:3]` 作为 13 bit lite 乘积参与加法树。
- 加法树位宽同步收窄：第一级部分和为 14 bit，第二级部分和为 15 bit，最终输出为 16 bit。
- Testbench 继续复用 `rtl/tb/equalizer_unfolding_n2_tb.v`。由于 lite RTL 文件名仍为 `equalizer_unfolding_n2.v`，`run_sim.py` 可以通过 `--rtl-file rtl/unfolding_lite/equalizer_unfolding_n2.v --frac-bits 7` 自动匹配原有 testbench 并按截断后的输出小数位进行浮点转换。

该版本的设计目标不是保持 bit-accurate 功能一致，而是用于后续 Vivado 综合中比较近似位宽裁剪对 LUT/FF/DSP 和时序指标的影响。仿真输出相对 MATLAB golden 结果大概率会出现 mismatch，但只要编译、运行和结果文件生成流程完整，即可用于 AI 辅助 PPA 优化实验的后续分析。
